Red wine analysis by Ilia Slobodchikov
========================================================
...
### 3. Fixed acidity, volatile acidity, and citric acid
...
### 6. Residual sugar, chlorides, sulphates, pH
...
### 1. Correlation between chemical properties
...
final_p_0 <- ggplot(data = wine, aes(y = alcohol, x = quality, group = quality)) +
   geom_boxplot(fill = "white", colour = "#0c1d55") +
   scale_x_continuous(breaks=seq(1,14,1)) +
...
### 3. Parameters which strongly influence the alcohol level in wine?
...
### 1. Chemical properties drill-down analysis 
...
# Plot fixed.acidity vs. density
  ggplot(data = wine, 
       ...
       geom_point(alpha = .6, position = position_jitter(h = 0), size = 2) +
       ...
       labs(title = 'Fixed acidity vs density correlation by quality', 
         ...
         # scale_color_brewer(palette = "Set5", type='seq') +
...
final_p_1 <- ggplot(data=wine, aes(x=alcohol, y=citric.acid)) +
    ...
    geom_jitter(alpha=.4, aes(size=quality.cut, color=quality.cut)) +
    ...
    scale_color_brewer(palette = "Set2", type = 'div') + 
    guides(color=guide_legend(title="Quality interval"), size=FALSE)
...
ggplot(data=wine, aes(x=alcohol, y=residual.sugar)) +
    ...
    geom_jitter(alpha=.3, aes(size=quality.cut, color=quality.cut)) +
    geom_vline(xintercept = wine.alcohol.mean, 
               linetype='longdash', color='black', alpha=.5) +
...
### 3. Parameters influencing alcohol level
